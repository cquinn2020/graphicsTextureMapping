...
std::vector<Triangle> triangleVector3;
...
std::string mainName = "Assignment3 - <Your Name>";
...
void Display()
{
	...
	if (isOpenGL)
	{
		...
		if (colorMode == 2)
		{
			for (int i = 0; i < triangleVector3.size(); i++)
				triangleVector3[i].RenderOpenGL(modelViewMatrix, projectionMatrix, isTextured);
		}
	...
	else
	{
		...
		{
			if (colorMode == 0)
				triangleVector[i].RenderCPU(modelViewMatrix, projectionMatrix, color, depth, texWidth, texHeight, isTextured, texture, textureMode);
			if (colorMode == 1)
				triangleVector2[i].RenderCPU(modelViewMatrix, projectionMatrix, color, depth, texWidth, texHeight, isTextured, texture, textureMode);
			if (colorMode == 2)
				triangleVector3[i].RenderCPU(modelViewMatrix, projectionMatrix, color, depth, texWidth, texHeight, isTextured, texture, textureMode);
		}
...
void CreateTriangleVector(std::vector<glm::vec3> &vertices, std::vector<glm::vec2> &texCoords)
{
	for (int i = 0; i < vertices.size() / 3; i++)
	{
		...
		if (texCoords.empty())
		{
			myTriangle = Triangle(vertices[i * 3 + 0], vertices[i * 3 + 1], vertices[i * 3 + 2], 0);
			myTriangle2 = Triangle(vertices[i * 3 + 0], vertices[i * 3 + 1], vertices[i * 3 + 2], 1);
			myTriangle3 = Triangle(vertices[i * 3 + 0], vertices[i * 3 + 1], vertices[i * 3 + 2], 2);
		}
		else
		{
			myTriangle = Triangle(vertices[i * 3 + 0], vertices[i * 3 + 1], vertices[i * 3 + 2],
								  texCoords[i * 3 + 0], texCoords[i * 3 + 1], texCoords[i * 3 + 2], 0);
			myTriangle2 = Triangle(vertices[i * 3 + 0], vertices[i * 3 + 1], vertices[i * 3 + 2],
								   texCoords[i * 3 + 0], texCoords[i * 3 + 1], texCoords[i * 3 + 2], 1);
			myTriangle3 = Triangle(vertices[i * 3 + 0], vertices[i * 3 + 1], vertices[i * 3 + 2],
								   texCoords[i * 3 + 0], texCoords[i * 3 + 1], texCoords[i * 3 + 2], 2);
		}

		triangleVector.push_back(myTriangle);
		triangleVector2.push_back(myTriangle2);
		triangleVector3.push_back(myTriangle3);
	}
}
